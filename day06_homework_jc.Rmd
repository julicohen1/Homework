...
setwd("~/UMD/Coursework/Winter 2019/Homework")

fam <- read_excel("dataTable.xlsx")
View(fam)

MOCA <- data.frame(subject = c(1:10, 12, 15:20),
                   m.score = rnorm(17, mean = 27, sd = 2.4))
View(MOCA)                   

factor(MOCA$subject)
MOCA$subject = factor(MOCA$subject)
fam$subject = factor(fam$subject)
factor(fam$subject)
...
fam.merge <- merge(fam, MOCA, all = TRUE)
view(fam.merge)


fam.full <- full_join(fam, MOCA, by = "subject")
view(fam.full)
...
fam.merge.2 <- merge(fam, MOCA, all = FALSE)
view(fam.merge)

fam.inner <- inner_join(fam, MOCA, by = "subject")
...
fam.left <- left_join(fam, MOCA, by = "subject")
view(fam.left)

fam.right <- right_join(fam, MOCA, by = "subject")
...
```{r}
#splitting the data into younger dataset and older dataset ()

fam$age_group = factor(fam$age_group)
fam$fam_group = factor(fam$fam_group)

levels(fam$age_group) <- c("Young", "Old")

fam.young <- filter(fam, age_group %in% c("Young"))
fam.old <- filter(fam, age_group %in% c("Old"))

levels(fam.young$age_group)
fam.young.clean <- droplevels(fam.young)
```
...
ggplot(fam, aes(log(polledResponse))) + geom_histogram(aes(fill = age_group)) + scale_x_log10(name = "Log Reaction Time")
...
fam.wide <- spread(fam, block, score)
view(fam.wide)

fam.long <- gather(fam.wide, score, block, 1:5) #something isn't right...
view(fam.long)
?gather
```
...
```{r}
fam
ggplot(fam, aes(lspan, polledResponse)) + 
  stat_summary(fun.y = "mean", geom = "point") + 
  scale_y_log10("Log Reaction Time")
...
ggplot(fam, aes(lspan, polledResponse, color = age_group)) + 
  stat_summary(fun.y = "mean", geom = "point") + 
  scale_y_log10("Log Reaction Time")
